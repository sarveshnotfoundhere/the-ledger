  ...
  return (
    <main>
      ...
      <section className="hero" id="top">
        <div className="hero-top"><div className="hero-label">THE FUTURE OF FINANCE</div><div className="hero-index">VOL. 01 / 2026</div></div>
        <div className="hero-main">
          <div className="hero-copy">
            ...
            <div className="hero-meta"><Link href="/accounting">ACCOUNTING</Link><Link href="/ai">AI</Link><Link href="/audit">AUDIT</Link><Link href="/fintech">FINTECH</Link></div>
          </div>
      ...
      <section className="section ledger-intro" id="accounting">
        <div className="ledger-intro-grid">
          ...
          <div className="ledger-intro-copy"><p className="lead">The Ledger is a premium digital publication exploring how technology is rewriting accounting, auditing, financial reporting and the profession itself.</p><p className="supporting">From artificial intelligence and robotic process automation to cloud accounting, connected data and digital audit, we examine what is changing, why it matters and what comes next.</p><div className="editorial-rule"></div></div>
        </div>
      </section>

      <section className="section latest" id="latest">
        <div className="section-heading"><div><div className="eyebrow">02 / EDITORIAL</div><h2>The Latest</h2></div><Link href="/latest" className="view-all">VIEW ALL →</Link></div>
        <div className="article-grid">
          {articles.map((article, index) => (
            <article className="article-card" key={article.title}>
              <Link href={`/story/${article.slug}`} className="article-image"><span>0{index + 1}</span></Link>
              <div className="article-category">{article.category}</div><h3><Link href={`/story/${article.slug}`}>{article.title}</Link></h3><p>{article.text}</p>
              <div className="article-meta"><span>{article.date}</span><span>{article.read}</span></div><Link className="read" href={`/story/${article.slug}`}>READ STORY →</Link>
            </article>
          ))}
        </div>
      </section>

      <section className="numbers section" id="data"><div className="eyebrow">03 / DATA</div><h2>Accounting<br /><span>in numbers.</span></h2><div className="number-grid"><div><strong>AI</strong><p>THE NEW ACCOUNTING LAYER</p></div><div><strong>RPA</strong><p>ROUTINE WORK, AUTOMATED</p></div><div><strong>24/7</strong><p>THE RISE OF CONTINUOUS AUDIT</p></div><div><strong>∞</strong><p>DATA WITHOUT BORDERS</p></div></div></section>
